...
"\n"
"<Commands> |命令\n"
"  a : Add files to archive |添加文件到文档\n"
"  b : Benchmark |标准检查程序\n"
"  d : Delete files from archive |从文档删除文件\n"
"  e : Extract files from archive (without using directory names) |从文档解压文件（无目录）\n"
"  h : Calculate hash values for files |计算文件哈希值\n"
"  i : Show information about supported formats |显示支持格式信息\n"
"  l : List contents of archive |显示文档列表\n"
"  rn : Rename files in archive |重命名文档文件\n"
"  t : Test integrity of archive |测试文档完整性\n"
"  u : Update files to archive |更新文件到文档\n"
"  x : eXtract files with full paths |完整路径解压文件\n"
"\n"
"<Switches> |选项\n"
"  -- : Stop switches parsing |关闭选项解析\n"
"  -ai[r[-|0]]{@listfile|!wildcard} : Include archives |包含文档\n"
"  -ax[r[-|0]]{@listfile|!wildcard} : eXclude archives |排除文档\n"
"  -ao{a|s|t|u} : set Overwrite mode |设置重写模式\n"
"  -an : disable archive_name field |禁用文档名域\n"
"  -bb[0-3] : set output log level |设置输出日志等级\n"
"  -bd : disable progress indicator |禁用过程指示器\n"
"  -bs{o|e|p}{0|1|2} : set output stream for output/error/progress line |设置 （输出/错误/过程）行输出\n"
"  -bt : show execution time statistics 显示执行时间统计\n"
"  -i[r[-|0]]{@listfile|!wildcard} : Include filenames 包含文件名列表\n"
"  -m{Parameters} : set compression Method | 设置压缩模式\n"
"    -mmt[N] : set number of CPU threads |设置CPU线程数\n"
"  -o{Directory} : set Output directory |设置输出目录\n"
#ifndef _NO_CRYPTO
"  -p{Password} : set Password	|设置密码\n"
#endif
"  -r[-|0] : Recurse subdirectories |递归子目录\n"
"  -sa{a|e|s} : set Archive name mode 设置文档名模式\n"
"  -scc{UTF-8|WIN|DOS} : set charset for for console input/output |设置控制台输入/输出字符集\n"
"  -scs{UTF-8|UTF-16LE|UTF-16BE|WIN|DOS|{id}} : set charset for list files |设置文件列表字符集\n"
"  -scrc[CRC32|CRC64|SHA1|SHA256|*] : set hash function for x, e, h commands |设置x，e，h命令的哈希函数\n"
"  -sdel : delete files after compression |压缩后删除文件\n"
"  -seml[.] : send archive by email |E-Mail发送文档\n"
"  -sfx[{name}] : Create SFX archive |创建SFX文档\n"
"  -si[{name}] : read data from stdin |由stdin读取数据\n"
"  -slp : set Large Pages mode |设置LargePages模式\n"
"  -slt : show technical information for l (List) command |l命令显示技术信息\n"
"  -snh : store hard links as links |储存hard links为links\n"
"  -snl : store symbolic links as links |储存symbolic links为links\n"
"  -sni : store NT security information |储存NT安全信息\n"
"  -sns[-] : store NTFS alternate streams |储存NTFS交替流\n"
"  -so : write data to stdout |写数据到stdout\n"
"  -spd : disable wildcard matching for file names |禁用文件名通配符匹配\n"
"  -spe : eliminate duplication of root folder for extract command |删除解压命令的根文件夹副本\n"
"  -spf : use fully qualified file paths |使用完全限定文件路径\n"
"  -ssc[-] : set sensitive case mode |设置敏感符号模式\n"
"  -ssw : compress shared files |压缩共享文件\n"
"  -stl : set archive timestamp from the most recently modified file |设置文档时间戳，为最近一次修改文件\n"
"  -stm{HexMask} : set CPU thread affinity mask (hexadecimal number) |设置CPU线程关联掩码（十六进制数）\n"
"  -stx{Type} : exclude archive type |排除文档格式\n"
"  -t{Type} : Set type of archive |设置文档格式\n"
"  -u[-][p#][q#][r#][x#][y#][z#][!newArchiveName] : Update options |更新选项\n"
"  -v{Size}[b|k|m|g] : Create volumes |创建卷\n"
"  -w[{path}] : assign Work directory. Empty path means a temporary directory |分配工作目录，空路径代表使用临时目录\n"
"  -x[r[-|0]]{@listfile|!wildcard} : eXclude filenames |排除文件\n"
"  -y : assume Yes on all queries |所有问题假设为“是”\n";
	...
	)
{
	...
	*/
	...
	//命令判断入口
	if (options.Command.CommandType == NCommandType::kInfo)					//显示支持格式信息
	{
	...
	else if (options.Command.CommandType == NCommandType::kBenchmark)		//标准测试
	{
		CStdOutStream &so = (g_StdStream ? *g_StdStream : g_StdOut);
